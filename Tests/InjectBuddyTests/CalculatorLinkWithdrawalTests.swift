...
///
/// **SHOWN RED BEFORE IT WAS TRUSTED** (§5.24), twice, and the FIRST ATTEMPT AT IT WAS
/// A CHECK THAT COULD NOT FAIL — which is worth more than either red run.
///
/// Flipping `isListed` so only Free T Index was withdrawn produced ONE failure, in
/// `testWithdrawnSetIsExactlyTheTwoTheOwnerNamed`, and left
/// `testNoBrowseSurfaceOffersAWithdrawnCalculator` green. That is not the check being
/// robust; it is the check being circular. Every browse surface DERIVES from `isListed`,
/// so measuring the surfaces against `isListed` can never disagree — it asks the
/// filter whether the filter ran. It has exactly one real failure mode: a surface that
/// bypasses the filter and enumerates `allCases` itself, which is what `DashboardScreen`
/// was doing an hour before this file existed.
///
/// So it was falsified by REPRODUCING THAT DEFECT — pointing the dashboard entry at
/// `CalculatorSlug.allCases` — and it went red naming both calculators and the surface:
///
///     CalculatorSlug.listedCases (DashboardScreen dialog) still links to
///     BMI, Free T Index
///
/// Reverted. §5.38's remedy in miniature: a check whose subject is chosen by the
/// predicate under test reports on nothing, and it looks identical to a check that
/// passes.
